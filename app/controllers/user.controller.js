...
exports.findPaginate = (req, res) => {
	...
	var client = createRedisClient();
	client.hget("users","users-"+req.query.page+"-"+req.query.limit, function (err, obj) {
		if(err || !obj)
		{
			User.paginate({}, { page: req.query.page, limit: req.query.limit })
				.then(users => {
					...
					client.hmset("users", ["users-"+req.query.page+"-"+req.query.limit, JSON.stringify(jsonResponse)] );
				}).catch(err => {
					res.status(500).send({
						message: err.message
					});
